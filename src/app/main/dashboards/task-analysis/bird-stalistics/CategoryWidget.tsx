...
function CategoryWidget() {
    ...
    function calculateSpeciesPercentages(birds) {
        ...
        birds.forEach(bird => {
            if (bird.category) {
                if (!speciesCounts[bird.category.name]) {
                    speciesCounts[bird.category.name] = 0;
                }
                speciesCounts[bird.category.name]++;
            }
        });
    ...
    return (
        ...
        <Paper className="flex flex-col flex-auto shadow rounded-2xl overflow-hidden p-24">
            <div className="flex flex-col sm:flex-row items-start justify-between">
                <Typography className="text-lg font-medium tracking-tight leading-6 truncate">Category</Typography>
                <div className="ml-8">
                    <Chip
                        size="small"
